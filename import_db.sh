...
mongosh --username "$MONGO_INITDB_ROOT_USERNAME" --password "$MONGO_INITDB_ROOT_PASSWORD" --authenticationDatabase admin "mongodb://localhost:27017/off" --eval 'db.products.createIndex({ _keywords: "text", product_name: "text" }, { weights: { product_name: 5, _keywords: 3}, name: "TextSearchIndex" });'
